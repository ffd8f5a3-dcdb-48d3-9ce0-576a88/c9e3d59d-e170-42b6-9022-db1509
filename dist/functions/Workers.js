...
const JobState_1 = __importDefault(require("../util/JobState"));
const Retry_1 = __importDefault(require("../util/Retry"));
const AdaptiveThrottler_1 = require("../util/AdaptiveThrottler");
class Workers {
    constructor(bot) {
        this.bot = bot;
        this.jobState = new JobState_1.default(this.bot.config);
    }
    // Daily Set
    async doDailySet(page, data) {
    ...
    // More Promotions
    async doMorePromotions(page, data) {
        ...
        if (data.promotionalItem) { // Convert and add the promotional item to the array
            morePromotions.push(data.promotionalItem);
        }
    ...
    async solveActivities(activityPage, activities, punchCard) {
        const activityInitial = activityPage.url(); // Homepage for Daily/More and Index for promotions
        const retry = new Retry_1.default(this.bot.config.retryPolicy);
        const throttle = new AdaptiveThrottler_1.AdaptiveThrottler();
            ...
            try {
                ...
                */
                ...
                if (typeLabel !== 'Unsupported') {
                    ...
                    const runWithTimeout = (p) => Promise.race([
                        p,
