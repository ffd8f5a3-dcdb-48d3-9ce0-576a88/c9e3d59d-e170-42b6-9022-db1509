...
const Totp_1 = require("../util/Totp");
...
class Login {
    constructor(bot) {
        ...
        this.passkeyHandled = false;
        this.noPromptIterations = 0;
        this.lastNoPromptLog = 0;
        this.bot = bot;
    }
    // --------------- Public API ---------------
    async login(page, email, password, totpSecret) {
        try {
    ...
    async getMobileAccessToken(page, email) {
        ...
        if (!code)
            throw this.bot.log(this.bot.isMobile, 'LOGIN-APP', 'OAuth code not received in time', 'error');
        const form = new URLSearchParams();
        form.append('grant_type', 'authorization_code');
        form.append('client_id', this.clientId);
        form.append('code', code);
        form.append('redirect_uri', this.redirectUrl);
        const req = { url: this.tokenUrl, method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, data: form.toString() };
    ...
    // --------------- Main Flow ---------------
    async performLoginFlow(page, email, password) {
    ...
    async inputPasswordOr2FA(page, password) {
    ...
    async handle2FA(page) {
        try {
            ...
            if (number) {
                await this.approveAuthenticator(page, number);
                return;
            }
    ...
    async fetchAuthenticatorNumber(page) {
        try {
    ...
    async approveAuthenticator(page, numberToPress) {
        for (let cycle = 0; cycle < 6; cycle++) { // max ~6 refresh cycles
            try {
                ...
                if (!refreshed) {
                    this.bot.log(this.bot.isMobile, 'LOGIN', 'Could not refresh authenticator code', 'warn');
                    return;
                }
    ...
    async handleSMSOrTotp(page) {
        // TOTP auto entry
        try {
            if (this.currentTotpSecret) {
                const code = (0, Totp_1.generateTOTP)(this.currentTotpSecret.trim());
        ...
        const code = await new Promise(res => rl.question('Enter 2FA code:\n', ans => { rl.close(); res(ans.trim()); }));
        await page.fill('input[name="otc"]', code);
        await page.keyboard.press('Enter');
        this.bot.log(this.bot.isMobile, 'LOGIN', '2FA code submitted');
    }
    // --------------- Verification / State ---------------
    async awaitRewardsPortal(page) {
        const start = Date.now();
        while (Date.now() - start < DEFAULT_TIMEOUTS.loginMaxMs) {
            await this.handlePasskeyPrompts(page, 'main');
            const u = new URL(page.url());
            if (u.hostname === LOGIN_TARGET.host && u.pathname === LOGIN_TARGET.path)
                break;
            await this.bot.utils.wait(1000);
        }
        ...
        try {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                if (u.hostname === 'www.bing.com' && u.pathname === '/') {
                    ...
                    if (ok || this.bot.isMobile) {
                        this.bot.log(this.bot.isMobile, 'LOGIN-BING', 'Bing verification passed');
                        break;
                    }
                }
                await this.bot.utils.wait(1000);
    ...
    async checkAccountLocked(page) {
        ...
        if (locked)
            throw this.bot.log(this.bot.isMobile, 'CHECK-LOCKED', 'Account locked by Microsoft (serviceAbuseLandingTitle)', 'error');
    }
    // --------------- Passkey / Dialog Handling ---------------
    async handlePasskeyPrompts(page, context) {
        let did = false;
        // Video heuristic
        ...
        if (!did) {
            ...
            if (!did) {
                ...
                if (await textBtn.isVisible().catch(() => false)) {
                    ...
                    did = true;
                    this.logPasskeyOnce('text fallback');
                }
            }
            if (!did) {
    ...
    // --------------- Security Detection ---------------
    async detectSignInBlocked(page) {
        if (this.bot.compromisedModeActive && this.bot.compromisedReason === 'sign-in-blocked')
            return true;
        try {
            let text = '';
            ...
            let matched = null;
            for (const p of SIGN_IN_BLOCK_PATTERNS) {
                if (p.re.test(lower)) {
                    matched = p.label;
                    break;
                }
    ...
    async tryRecoveryMismatchCheck(page, email) { try {
        await this.detectAndHandleRecoveryMismatch(page, email);
    }
    catch { /* ignore */ } }
    async detectAndHandleRecoveryMismatch(page, email) {
        try {
            const recoveryEmail = this.bot.currentAccountRecoveryEmail;
            if (!recoveryEmail || !/@/.test(recoveryEmail))
                return;
            const accountEmail = email;
            const parseRef = (val) => { const [l, d] = val.split('@'); return { local: l || '', domain: (d || '').toLowerCase(), prefix2: (l || '').slice(0, 2).toLowerCase() }; };
            ...
            for (let i = 0; i < liCount && i < 12; i++) {
                const t = (await li.nth(i).textContent().catch(() => ''))?.trim() || '';
                if (t && /@/.test(t))
                    candidates.push(t);
            }
            ...
            for (let i = 0; i < xpCount && i < 12; i++) {
                const t = (await xp.nth(i).textContent().catch(() => ''))?.trim() || '';
                if (t && t.length < 300)
                    candidates.push(t);
            }
            // Normalize
    ...
    async switchToPasswordLink(page) {
        try {
    ...
    // --------------- Incident Helpers ---------------
    async sendIncidentAlert(incident, severity = 'warn') {
        ...
        if (incident.docsUrl)
            lines.push(`Docs: ${incident.docsUrl}`);
    ...
    startCompromisedInterval() {
        if (this.compromisedInterval)
            clearInterval(this.compromisedInterval);
        this.compromisedInterval = setInterval(() => {
            try {
                this.bot.log(this.bot.isMobile, 'SECURITY', 'Account in security standby. Review before proceeding. Security check by @Light', 'warn');
            }
            catch { /* ignore */ }
        }, 5 * 60 * 1000);
    }
    async saveIncidentArtifacts(page, slug) {
        try {
            const base = path_1.default.join(process.cwd(), 'diagnostics', 'security-incidents');
            await fs_1.default.promises.mkdir(base, { recursive: true });
            const ts = new Date().toISOString().replace(/[:.]/g, '-');
            const dir = path_1.default.join(base, `${ts}-${slug}`);
            await fs_1.default.promises.mkdir(dir, { recursive: true });
            try {
                await page.screenshot({ path: path_1.default.join(dir, 'page.png'), fullPage: false });
            }
            catch { /* ignore */ }
            try {
                const html = await page.content();
                await fs_1.default.promises.writeFile(path_1.default.join(dir, 'page.html'), html);
            }
            catch { /* ignore */ }
