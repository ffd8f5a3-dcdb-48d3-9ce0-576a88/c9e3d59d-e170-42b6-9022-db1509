...
class Search extends Workers_1.Workers {
    ...
    async doSearch(page, data) {
        ...
        if (!googleSearchQueries.length || googleSearchQueries.length < 10) {
            ...
            try {
                const local = await Promise.resolve().then(() => __importStar(require('../queries.json')));
                // Flatten & sample
                ...
                googleSearchQueries = sampled.map((x) => ({ topic: x.queries[0] || x.title, related: x.queries.slice(1) }));
            }
            catch (e) {
        ...
        googleSearchQueries = googleSearchQueries.filter(q => {
            if (seen.has(q.topic.toLowerCase()))
                return false;
        ...
        if (missingPoints > 0) {
            ...
            while (missingPoints > 0 && fallbackRounds <= extraRetries) {
                const query = googleSearchQueries[i++];
                if (!query)
                    break;
    ...
    async bingSearch(searchPage, query) {
        const platformControlKey = (0, os_1.platform)() === 'darwin' ? 'Meta' : 'Control';
    ...
    async getGoogleTrends(geoLocale = 'US') {
        const queryTerms = [];
