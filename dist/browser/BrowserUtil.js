    ...
    async tryDismissAllMessages(page) {
        ...
        for (let round = 0; round < attempts; round++) {
            ...
            if (dismissedThisRound === 0)
                break; // nothing new dismissed -> stop early
        }
    }
    ...
    async humanizePage(page) {
        try {
            ...
            if (range && typeof range.min !== 'undefined' && typeof range.max !== 'undefined') {
                try {
                    const ms = (await Promise.resolve().then(() => __importStar(require('ms')))).default;
