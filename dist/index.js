...
const Humanizer_1 = __importDefault(require("./util/Humanizer"));
const BanDetector_1 = require("./util/BanDetector");
// Main bot class
class MicrosoftRewardsBot {
    constructor(isMobile) {
        ...
        this.humanizer = new Humanizer_1.default(this.utils, this.config.humanization);
    ...
    async run() {
        ...
        if (hbFile) {
            try {
                const dir = path_1.default.dirname(hbFile);
                if (!fs_1.default.existsSync(dir))
                    fs_1.default.mkdirSync(dir, { recursive: true });
                fs_1.default.writeFileSync(hbFile, String(Date.now()));
        ...
        if (this.buyMode.enabled) {
            ...
            (0, Logger_1.log)('main', 'BUY-MODE', `Buy mode ENABLED${targetInfo}. We'll open 2 tabs: (1) a monitor tab that auto-refreshes to track points, (2) your browsing tab to redeem/purchase freely.`, 'log', 'green');
            (0, Logger_1.log)('main', 'BUY-MODE', 'The monitor tab may refresh every ~10s. Use the other tab for your actions; monitoring is passive and non-intrusive.', 'log', 'yellow');
    ...
    async runBuyMode() {
        try {
            ...
            if (!account)
                throw new Error('No account available for buy mode');
            this.isMobile = false;
            ...
            const sendSpendNotice = async (delta, nowPts, cumulativeSpent) => {
                try {
                    const { ConclusionWebhook } = await Promise.resolve().then(() => __importStar(require('./util/ConclusionWebhook')));
            ...
            let spent = 0;
            const cfgAny = this.config;
            const buyModeConfig = cfgAny['buyMode'];
...
`;
        try {
            ...
            if (this.buyMode.enabled) {
                ...
                if (!schedEnabled) {
                    ...
                    let timeShown = '';
                    const srec = sched;
                    const useAmPmVal = typeof srec['useAmPm'] === 'boolean' ? srec['useAmPm'] : undefined;
                    ...
                    if (useAmPmVal === true) {
                        ...
                        else if (time12Val && time12Val.trim()) {
                            formatName = 'AM/PM';
                            timeShown = time12Val;
                        }
                        else {
                            formatName = 'legacy';
                            timeShown = sched.time || '09:00';
                        }
                    }
        ...
        cluster_1.default.on('exit', (worker, code) => {
            ...
            try {
                ...
                if (cr?.restartFailedWorker && code !== 0) {
                    const attempts = worker._restartAttempts || 0;
                    if (attempts < (cr.restartFailedWorkerAttempts ?? 1)) {
                        worker._restartAttempts = attempts + 1;
                        (0, Logger_1.log)('main', 'CRASH-RECOVERY', `Respawning worker (attempt ${attempts + 1})`, 'warn', 'yellow');
                        const newW = cluster_1.default.fork();
    ...
    async runTasks(accounts) {
        for (const account of accounts) {
            ...
            if (this.globalStandby.active) {
                (0, Logger_1.log)('main', 'SECURITY', `Global standby active (${this.globalStandby.reason || 'security-issue'}). Not proceeding to next accounts until resolved.`, 'warn', 'yellow');
                break;
            }
            // Optional global stop after first ban
            if (this.config?.humanization?.stopOnBan === true && this.bannedTriggered) {
                (0, Logger_1.log)('main', 'TASK', `Stopping remaining accounts due to ban on ${this.bannedTriggered.email}: ${this.bannedTriggered.reason}`, 'warn');
                break;
            }
            ...
            try {
                const windows = this.config?.humanization?.allowedWindows;
                if (Array.isArray(windows) && windows.length > 0) {
                    const waitMs = this.computeWaitForAllowedWindow(windows);
                    if (waitMs > 0) {
                        (0, Logger_1.log)('main', 'HUMANIZATION', `Waiting ${Math.ceil(waitMs / 1000)}s until next allowed window before starting ${account.email}`, 'warn');
            ...
            if (this.config.parallel) {
                ...
                const desktopPromise = this.Desktop(account).catch(e => {
                    ...
                    (0, Logger_1.log)(false, 'TASK', `Desktop flow failed early for ${account.email}: ${msg}`, 'error');
                    const bd = (0, BanDetector_1.detectBanReason)(e);
                    if (bd.status) {
                        banned.status = true;
                        banned.reason = bd.reason.substring(0, 200);
                ...
                const mobilePromise = mobileInstance.Mobile(account).catch(e => {
                    const msg = e instanceof Error ? e.message : String(e);
                    (0, Logger_1.log)(true, 'TASK', `Mobile flow failed early for ${account.email}: ${msg}`, 'error');
                    const bd = (0, BanDetector_1.detectBanReason)(e);
                    if (bd.status) {
                        banned.status = true;
                        banned.reason = bd.reason.substring(0, 200);
                ...
                if (!banned.status && !this.compromisedModeActive) {
                    ...
                    (0, Logger_1.log)(true, 'TASK', `Skipping mobile flow for ${account.email} due to ${why}`, 'warn');
                }
            }
            ...
            const baselines = [];
            if (desktopInitial)
                baselines.push(desktopInitial);
            if (mobileInitial)
                baselines.push(mobileInitial);
            let initialTotal = 0;
            if (baselines.length === 1)
                initialTotal = baselines[0];
            else if (baselines.length === 2)
                initialTotal = Math.min(baselines[0], baselines[1]);
            // Fallback if both missing
            if (initialTotal === 0 && (desktopInitial || mobileInitial))
                initialTotal = desktopInitial || mobileInitial || 0;
            ...
            await (0, Logger_1.log)('main', 'MAIN-WORKER', `Completed tasks for account ${account.email}`, 'log', 'green');
        }
        await (0, Logger_1.log)(this.isMobile, 'MAIN-PRIMARY', 'Completed tasks for ALL accounts', 'log', 'green');
        // Extra diagnostic summary when verbose
        if (process.env.DEBUG_REWARDS_VERBOSE === '1') {
            for (const summary of this.accountSummaries) {
                (0, Logger_1.log)('main', 'SUMMARY-DEBUG', `Account ${summary.email} collected D:${summary.desktopCollected} M:${summary.mobileCollected} TOTAL:${summary.totalCollected} ERRORS:${summary.errors.length ? summary.errors.join(';') : 'none'}`);
            }
        }
        ...
        if (this.compromisedModeActive || this.globalStandby.active) {
            (0, Logger_1.log)('main', 'SECURITY', 'Compromised or banned detected. Global standby engaged: we will NOT proceed to other accounts until resolved. Keeping process alive. Press CTRL+C to exit when done. Security check by @Light', 'warn', 'yellow');
            // Periodic heartbeat
            setInterval(() => {
                (0, Logger_1.log)('main', 'SECURITY', 'Still in standby: session(s) held open for manual recovery / review...', 'warn', 'yellow');
            }, 5 * 60 * 1000);
            return;
        }
        ...
        if (this.config.clusters > 1 && !cluster_1.default.isPrimary) {
            ...
            if (this.heartbeatFile) {
                try {
                    if (fs_1.default.existsSync(this.heartbeatFile))
                        fs_1.default.unlinkSync(this.heartbeatFile);
                }
                catch { /* ignore */ }
            }
            // After conclusion, run optional auto-update
            await this.runAutoUpdate().catch(() => { });
        }
        process.exit();
    }
    /** Send immediate ban alert if configured. */
    async handleImmediateBanAlert(email, reason) {
        try {
            const h = this.config?.humanization;
            if (!h || h.immediateBanAlert === false)
                return;
            const { ConclusionWebhook } = await Promise.resolve().then(() => __importStar(require('./util/ConclusionWebhook')));
            ...
            (0, Logger_1.log)('main', 'ALERT', `Failed to send ban alert: ${e instanceof Error ? e.message : e}`, 'warn');
        }
    }
    /** Compute milliseconds to wait until within one of the allowed windows (HH:mm-HH:mm). Returns 0 if already inside. */
    computeWaitForAllowedWindow(windows) {
        const now = new Date();
        const minsNow = now.getHours() * 60 + now.getMinutes();
        let nextStartMins = null;
    ...
    async Desktop(account) {
        (0, Logger_1.log)(false, 'FLOW', 'Desktop() invoked');
        const browser = await this.browserFactory.createBrowser(account.proxy, account.email);
        ...
        if (this.compromisedModeActive) {
            ...
            (0, Logger_1.log)(this.isMobile, 'SECURITY', `Account flagged as compromised (${reason}). Leaving the browser open and skipping all activities for ${account.email}. Security check by @Light`, 'warn', 'yellow');
            try {
                const { ConclusionWebhook } = await Promise.resolve().then(() => __importStar(require('./util/ConclusionWebhook')));
            ...
            try {
                await (0, Load_1.saveSessionData)(this.config.sessionPath, this.homePage.context(), account.email, this.isMobile);
            }
            catch { /* ignore */ }
        ...
        if (this.pointsCanCollect === 0) {
            ...
            (0, Logger_1.log)(this.isMobile, 'MAIN-POINTS', `Breakdown (desktop): dailySet=${browserEnarablePoints.dailySetPoints} search=${browserEnarablePoints.desktopSearchPoints} promotions=${browserEnarablePoints.morePromotionsPoints}`);
            (0, Logger_1.log)(this.isMobile, 'MAIN-POINTS', 'All desktop earnable buckets are zero. This usually means: tasks already completed today OR the daily reset has not happened yet for your time zone. If you still want to force run activities set execution.runOnZeroPoints=true in config.', 'log', 'yellow');
        }
    ...
    async Mobile(account) {
        (0, Logger_1.log)(true, 'FLOW', 'Mobile() invoked');
        const browser = await this.browserFactory.createBrowser(account.proxy, account.email);
        this.homePage = await browser.newPage();
        (0, Logger_1.log)(this.isMobile, 'MAIN', 'Starting browser');
        ...
        if (this.compromisedModeActive) {
            const reason = this.compromisedReason || 'security-issue';
            (0, Logger_1.log)(this.isMobile, 'SECURITY', `Account flagged as compromised (${reason}). Leaving mobile browser open and skipping mobile activities for ${account.email}. Security check by @Light`, 'warn', 'yellow');
            try {
                const { ConclusionWebhook } = await Promise.resolve().then(() => __importStar(require('./util/ConclusionWebhook')));
        ...
        if (this.pointsCanCollect === 0) {
            (0, Logger_1.log)(this.isMobile, 'MAIN-POINTS', `Breakdown (mobile): browserSearch=${browserEnarablePoints.mobileSearchPoints} appTotal=${appEarnablePoints.totalEarnablePoints}`);
            (0, Logger_1.log)(this.isMobile, 'MAIN-POINTS', 'All mobile earnable buckets are zero. Causes: mobile searches already maxed, daily set finished, or daily rollover not reached yet. You can force execution by setting execution.runOnZeroPoints=true.', 'log', 'yellow');
        }
        // If runOnZeroPoints is false and 0 points to earn, don't continue
        if (!this.config.runOnZeroPoints && this.pointsCanCollect === 0) {
            (0, Logger_1.log)(this.isMobile, 'MAIN', 'No points to earn and "runOnZeroPoints" is set to "false", stopping!', 'log', 'yellow');
        ...
        (0, Logger_1.log)(this.isMobile, 'MAIN-POINTS', `The script collected ${afterPointAmount - initialPoints} points today`);
        // Close mobile browser
        await this.browser.func.closeBrowser(browser, account.email);
        return {
            initialPoints: initialPoints,
    ...
    async sendConclusion(summaries) {
        const { ConclusionWebhook } = await Promise.resolve().then(() => __importStar(require('./util/ConclusionWebhook')));
        ...
        for (const s of summaries) {
            ...
            if (s.banned?.status)
                lines.push(`  Banned: ${s.banned.reason || 'detected by heuristics'}`);
            if (s.errors.length)
                lines.push(`  Errors: ${s.errors.slice(0, 2).join(' | ')}`);
        ...
        if (omitted > 0 && embeds.length > 0) {
            ...
            const noteField = { name: 'Note', value: `And ${omitted * fieldsPerEmbed} more account entries not shown due to Discord limits.`, inline: false };
        ...
        try {
            ...
            (0, Logger_1.log)('main', 'REPORT', `Saved report to ${file}`);
        }
        catch (e) {
            (0, Logger_1.log)('main', 'REPORT', `Failed to save report: ${e instanceof Error ? e.message : e}`, 'warn');
        }
        ...
        try {
            ...
            (0, Logger_1.log)('main', 'REPORT', `Failed diagnostics cleanup: ${e instanceof Error ? e.message : e}`, 'warn');
        }
    }
    /** Reserve one diagnostics slot for this run (caps captures). */
    tryReserveDiagSlot(maxPerRun) {
        if (this.diagCount >= Math.max(0, maxPerRun || 0))
            return false;
        this.diagCount += 1;
        return true;
    }
    /** Delete diagnostics folders older than N days under ./reports */
    async cleanupOldDiagnostics(retentionDays) {
        const base = path_1.default.join(process.cwd(), 'reports');
        ...
        for (const e of entries) {
            ...
            const parts = name.split('-').map((n) => parseInt(n, 10));
    ...
    async runAutoUpdate() {
        ...
        const scriptAbs = path_1.default.join(process.cwd(), scriptRel);
        if (!fs_1.default.existsSync(scriptAbs))
            return;
        const args = [];
        ...
        await new Promise((resolve) => {
            const child = (0, child_process_1.spawn)(process.execPath, [scriptAbs, ...args], { stdio: 'inherit' });
            child.on('close', () => resolve());
            child.on('error', () => resolve());
        });
    }
    /** Public entry-point to engage global security standby from other modules (idempotent). */
    async engageGlobalStandby(reason, email) {
        try {
            if (this.globalStandby.active)
                return;
    ...
    async sendGlobalSecurityStandbyAlert(email, reason) {
        try {
            const { ConclusionWebhook } = await Promise.resolve().then(() => __importStar(require('./util/ConclusionWebhook')));
            ...
            (0, Logger_1.log)('main', 'ALERT', `Failed to send standby alert: ${e instanceof Error ? e.message : e}`, 'warn');
        }
    }
}
...
function shortErr(e) {
    ...
    if (e instanceof Error)
        return e.message.substring(0, 120);
    const s = String(e);
...
async function main() {
    ...
    const attachHandlers = () => {
        process.on('unhandledRejection', (reason) => {
            (0, Logger_1.log)('main', 'FATAL', 'UnhandledRejection: ' + (reason instanceof Error ? reason.message : String(reason)), 'error');
            gracefulExit(1);
        });
        process.on('uncaughtException', (err) => {
            (0, Logger_1.log)('main', 'FATAL', 'UncaughtException: ' + err.message, 'error');
            gracefulExit(1);
        });
    ...
    const gracefulExit = (code) => {
        try {
            rewardsBot['heartbeatTimer'] && clearInterval(rewardsBot['heartbeatTimer']);
        }
        catch { /* ignore */ }
        if (config?.crashRecovery?.autoRestart && code !== 0) {
            ...
            if (crashState.restarts < max) {
                ...
                (0, Logger_1.log)('main', 'CRASH-RECOVERY', `Scheduling restart in ${backoff}ms (attempt ${crashState.restarts + 1}/${max})`, 'warn', 'yellow');
                setTimeout(() => {
                    crashState.restarts++;
                    bootstrap();
                }, backoff);
    ...
    const bootstrap = async () => {
        try {
            ...
            (0, Logger_1.log)('main', 'MAIN-ERROR', 'Fatal during run: ' + (e instanceof Error ? e.message : e), 'error');
            gracefulExit(1);
        }
    };
    attachHandlers();
